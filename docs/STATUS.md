...
| macros & beacons | 🟡 works, polishing | two warm CPython authority domains: BOUND-by-default capabilities and explicit `# neuron: raw` full Python, plus ask/notify, persistent state, authority-safe macro composition, and a source-preserving block builder. still polishing the API/docs and long-run runtime ergonomics. |
...
| installing & updating | 🟡 works, one desk | a release archive unpacks anywhere writable and keeps config beside the binary, and an updater script per platform (`skills/neuron-lazy-update/scripts/`) does the risky parts: checksum, provenance attestation, back up what it overwrites, verify the version afterwards, roll back. what's actually been exercised differs by platform, and it's worth knowing which: the **windows** path has been run end to end on my desk — package built, installed to a clean folder, binaries checked for missing runtime DLLs, and the installed CLI drove real hardware. the **linux** path has had its logic tested against synthetic archives (install, update, rollback, refused downgrade, refused tampered checksum, config left untouched) but has never been run on a real linux machine by anyone, and the udev step it prints has never been followed on one. treat the first real linux install as the test it is. |
| cross-platform: linux | 🟠 built, hardware-unverified | the complete workspace, including the app, builds and is tested on linux. WSL2 has no razer device on the other end, so this proves the code and tests run, not that device writes land. the hidraw transport uses sysfs enumeration, `HIDIOCGFEATURE`/`HIDIOCSFEATURE`, and `flock`; releases include the app, CLI, and udev rule. no razer device has been plugged into a linux box running neuron yet. treat the first hardware run as the real test, and [tell me what happened](https://github.com/worflor/neuron/issues). |
| cross-platform: the linux GUI | 🟠 built, hardware-unverified | the Slint app window, device settings, lighting editor, GTK tray menu and best-effort global hotkeys are included in the linux archive. evdev/uinput now drives live binds, scoped remaps and synthesized actions; GTK/Cairo renders the overlay; PulseAudio-compatible `pactl`/`parec` drives audio controls and meters. the audio control path was exercised against an isolated PulseAudio server. WSL2 has no `/dev/input`, `/dev/uinput` or Razer device, so remaps, gesture capture and overlay placement still require a native desktop and hardware run. the app stays disarmed when input access is missing. |
...
**macros & beacons.** the runtime is real and fast: BOUND and RAW each have their own warm CPython process, imports stay paid, and a RAW crash cannot take the app or BOUND macros down with it. new code starts on the brokered BOUND surface; full Python is one explicit source directive away. what's still thin is the polished API/reference and long-run ergonomics rather than the authority model itself.
