...
| reliability & recovery | 🟡 built, unproven | a flight recorder and crash log turn silent failures into visible state, and the diagnostics bench proves the app's health read-only (it's what CI runs). on a crash or hang, windows can relaunch it ("phoenix") — honest caveat: it hasn't crashed on me yet, so that auto-restart has genuinely never had to fire. wired, not battle-tested. |
