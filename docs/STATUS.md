...
| cross-platform: the linux GUI | 🟠 built, partial backends | the Slint app window, device settings, lighting editor, GTK tray menu and best-effort global hotkeys are included in the linux archive. live input capture/synthesis, overlays, and audio controls still need linux backends; the app does not claim those paths are armed. |
