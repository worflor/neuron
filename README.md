...
- **[writes that verify themselves](docs/GDD.md#writes-that-verify-themselves).** every write is volatile first (`NOSTORE`), then re-reads the matching getter and confirms the bytes it sent actually landed. a mismatch is a hard error, never a silent success. when there's no opcode it trusts, it refuses rather than guessing at your hardware.
