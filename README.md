...
the **linux GUI** runs the same slint window, device settings and lighting editor, with a GTK tray menu and best-effort global hotkeys. live binds and remaps use evdev and uinput; the overlay uses GTK/Cairo; audio controls and meters use the PulseAudio-compatible server available with PulseAudio or PipeWire. it needs GTK 3, AppIndicator, libxdo, libxkbcommon-x11 (`libxkbcommon-x11-0` on Ubuntu), `pactl`, and `parec` at runtime (`pulseaudio-utils` supplies the last two on Ubuntu). load the `uinput` module and install the bundled udev rule to grant the active desktop user access to Razer hidraw and input nodes and `/dev/uinput`. the app stays disarmed when input access is unavailable. these new linux input and overlay paths still need a real-device desktop run.
