...
**macros are unsandboxed on purpose.** the bundled CPython runs *your* macro files as a subprocess with full `ctypes` / `subprocess` / socket / filesystem access. this is the feature, not an oversight — a macro can do anything a program you ran can do. the friendly helpers respect the arm gate; raw `ctypes` past them does not. don't run a macro file you didn't read, exactly like any other script. the sidecar being a separate process is a *reliability* boundary (a segfaulting macro can't take the app down), not a security one.
