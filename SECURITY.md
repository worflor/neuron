...
- **the native Chroma face is shared memory, not a socket.** games reach Razer's SDK through `Global\` shared-memory objects. creating those sections requires `SeCreateGlobalPrivilege`. The optional `neuron-chroma-broker` runs from a protected Program Files directory at high integrity and only creates and holds the fixed Chroma sections. The limited tray process adopts them and owns parsing, arbitration and device output. The broker has no macro, input, network or HID path. The sections grant **full access to Everyone** (`D:(A;;GA;;;WD)`), because that is what games expect to open. Without the broker, Chroma REST remains available.

that permission isn't a choice i get to make — a game opens these with `FILE_MAP_ALL_ACCESS`, which includes the standard write-DACL/write-owner rights, so a tighter descriptor fails the open and the game never paints. neuron uses fixed section sizes, and parse loops are bounds-checked. Do not run the whole user-writable app as administrator to recover native Chroma: RAW Python and the executable itself then inherit that authority.
